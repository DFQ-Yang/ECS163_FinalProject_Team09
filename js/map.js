...
export function draw_map(){
    d3.csv(map_config.data_path).then(async raw_data => {
        ...
        function draw_graph(){
            d3.selectAll('.graph_map').selectAll("*").remove();

            const rect = document.querySelector('.graph_map').getBoundingClientRect();
            ...
            const svg = d3.selectAll(".graph_map").append("svg").attr("viewBox", [0, 0, width, height])
